...
async def _setup_shared_hvac_mode_helper(
    hass: HomeAssistant, initial_mode: HVACMode = HVACMode.HEAT
) -> None:
    """Create a real input_select helper for shared HVAC mode tests."""
    assert await async_setup_component(
        ...
        "input_select",
        {
            "input_select": {
                "shared_hvac_mode": {
                    "options": [HVACMode.OFF, HVACMode.HEAT, HVACMode.COOL],
...
async def test_shared_hvac_mode_preserves_locally_off_zones(
    ...
    await _setup_shared_hvac_mode_helper(hass, HVACMode.HEAT)
...
async def test_shared_hvac_mode_restore_keeps_participating_zone_off_until_global_mode_returns(
    ...
    hass.config.units = METRIC_SYSTEM
    setup_sensor(hass, 22.0)
    await _setup_shared_hvac_mode_helper(hass, HVACMode.OFF)
    _setup_test_switches(
        hass,
        "switch.zone_1_heat",
        "switch.zone_1_cool",
...
async def test_shared_hvac_mode_restore_off_state_wins_over_initial_heat(
    hass: HomeAssistant,
) -> None:
    """A restored local off state must not be masked by initial_hvac_mode."""
    hass.config.units = METRIC_SYSTEM
    setup_sensor(hass, 22.0)
    await _setup_shared_hvac_mode_helper(hass, HVACMode.COOL)
    _setup_test_switches(
        hass,
        "switch.zone_1_heat",
        "switch.zone_1_cool",
